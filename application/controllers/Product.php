...
class Product extends CI_Controller {
    ...
    public function index()
    {
        ...
        $this->data['colors'] = $q->result_array();

        $this->db->select("color_id");
        $this->db->where('product_id',  $product['id']);
        //   $this->db->order_by("priority", "asc");
        $q = $this->db->get('product_colors');
        $this->data['product_colors'] = $q->result_array();
        $this->data['product_colors_arr'] = array_column($this->data['product_colors'], 'color_id');
        ...
        $this->db->where('product_id',  $product['id']);
        $q = $this->db->get('product_accessories');
        $this->data['product_accessories'] = $q->result_array();
        $this->data['product_accessories_arr'] = array_column($this->data['product_accessories'], 'accessory_id');

        $this->db->order_by("priority", "asc");
        $q = $this->db->get('accessories');
        $this->data['accessories'] = $q->result_array();


        $this->db->where('product_id',  $product['id']);
        $this->db->order_by("priority", "desc");
        $q = $this->db->get('product_sizes');
        $this->data['product_sizes'] = $q->result_array();

        $this->db->where('product_id',  $product['id']);
        $this->db->order_by("priority", "desc");
        $q = $this->db->get('product_sizes');
        $this->data['product_size_first'] = $q->row_array();
        ...
        $this->db->limit('6');
        $q = $this->db->get('products');
        $this->data['products_related'] = $q->result_array();

        $this->data['relatedProductsView'] = '';
        foreach($this->data['products_related'] as $product){
            $this->data['relatedProductsView'] .= $this->product_model->create($product, 1);
        }
